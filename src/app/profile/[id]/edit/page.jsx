...
export default function Edit() {
      ...
      try {
        ...
        if (Array.isArray(data.lang_spoken) && data.lang_spoken.length > 0) {
          userLangs = data.lang_spoken.map((langCode) => ({
            code: langCode,
            name:
              allLanguages.find((lang) => lang.code === langCode)?.name ||
              langCode,
          }));
        }
  ...
  useEffect(() => {
    if (allLanguages.length > 0 && userData?.languages) {
      ...
      const needsUpdate = userData.languages.some(
        (userLang) =>
  ...
  const cardClass =
    "mb-8 bg-[#1a1333] rounded-lg shadow-lg p-6 animate-fadeIn transition-all duration-700 ease-out backdrop-blur-md border-2 border-transparent hover:border-gradient-to-r from-purple-400 to-pink-400 hover:scale-105 hover:shadow-2xl";
  const cardInnerClass =
    "bg-[#24194a] p-3 rounded mb-2 hover:scale-105 hover:shadow-xl transition-transform duration-300";
  const buttonClass =
    "px-4 py-2 bg-purple-600 hover:bg-purple-800 rounded text-white font-semibold transition-transform duration-200 hover:scale-105 hover:shadow-lg";
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#1a1333] to-[#2d1a4d] text-white p-6 pt-28 flex">
        ...
        {/* Sidebar */}
        <aside
          className="w-64 mr-8 hidden md:block animate-fadeInUp glass-sidebar transition-all duration-500"
          style={{ animationDelay: "0.2s", minWidth: "260px" }}
        >
          <div
            className="flex flex-col items-center gap-6 py-8 px-6 bg-white/10 rounded-xl shadow-xl border border-white/10 backdrop-blur-md transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 animate-popIn"
                ...
                <>
                  <div className="flex items-center gap-2 justify-between">
                    ...
                    <span className="px-2 py-0.5 rounded bg-gray-800 text-gray-100 font-medium tracking-wide min-w-[60px] text-center">
                      {userData?.experience ||
                        userData?.experience_display ||
                        "N/A"}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 justify-between">
                    <span className="text-gray-400">Avg. Rating</span>
                    <span className="px-2 py-0.5 rounded bg-gray-800 text-yellow-300 font-semibold flex items-center gap-1 min-w-[60px] justify-center">
                      {userData?.avgRating ?? "N/A"}{" "}
                      <svg
                  ...
                  <div className="flex items-center gap-2 justify-between">
                    ...
                    <span className="px-2 py-0.5 rounded bg-gray-800 text-blue-200 font-semibold min-w-[60px] text-center">
                      {userData?.ongoingOrders || 0}
                    </span>
                  </div>
                  <div className="flex items-center gap-2 justify-between">
                    <span className="text-gray-400">Completed Orders</span>
                    <span className="px-2 py-0.5 rounded bg-gray-800 text-green-200 font-semibold min-w-[60px] text-center">
                  ...
                  <span className="px-2 py-0.5 rounded bg-gray-800 text-purple-300 font-medium text-center max-w-[120px] truncate">
                    {(() => {
                      ...
                      if (
                        ...
                        languageNames = userData.languages.map(
                          (lang) => lang.name
                        );
                      }
                      ...
                      return (
                        <div className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none group-hover:pointer-events-auto z-10">
                          <div className="bg-[#1a1333] border border-purple-500/30 rounded-lg shadow-2xl p-3 min-w-[200px] max-w-[300px] backdrop-blur-md">
                            <div className="text-xs font-semibold text-purple-300 mb-2 border-b border-purple-500/30 pb-1">
                              All Languages:
                            </div>
                            <div className="space-y-1">
                              {languageNames.map((lang, index) => (
                                <div
                                  key={index}
                                  className="text-xs text-gray-200 flex items-center gap-2"
                                >
        ...
        <div className="flex-1 max-w-4xl mx-auto">
          ...
          <div
            className={
              cardClass +
              " animate-fadeInUp animate-popIn transition-all duration-500 hover:shadow-2xl hover:-translate-y-1"
            }
            style={{ animationDelay: "0.3s" }}
          >
            <h1
              className="text-2xl font-bold text-purple-400 mb-2 animate-popIn"
          ...
          >
            <h2
              className="text-xl font-bold text-purple-400 mb-4 animate-popIn"
            ...
            <div className="flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="flex flex-col gap-3">
                <label
                  ...
                  className={
                    buttonClass +
                    " text-sm cursor-pointer w-fit flex items-center gap-2"
                  }
                >
          ...
          >
            ...
            <div className="space-y-2">
              ...
              <div className="flex justify-between items-center">
                <span
                  className={`text-sm ${
                    userData.about?.length > 500
                      ? "text-red-400"
                      : "text-purple-300"
                  }`}
                >
                  Maximum 500 characters allowed. {userData.about?.length || 0}
                  /500 characters
                </span>
              </div>
            </div>
                ...
                {userData.languages.length > 0 ? (
                  userData.languages.map((language, index) => (
                    <div
                      ...
                      className={
                        cardInnerClass +
                        " flex items-center justify-between p-4 bg-[#2d1a4d]"
                      }
                    >
