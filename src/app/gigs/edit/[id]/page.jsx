...
export default function EditGig() {
  ...
  useEffect(() => {
    ...
    async function fetchGig() {
      try {
        ...
        const initialCategory = Array.isArray(data.categories) && data.categories.length > 0
          ? { value: data.categories[0].id, label: data.categories[0].name }
          : null;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await gigService.updateGig(gigId, gigData);
  ...
  // Utility function to compare form and category
  function isFormChanged() {
    if (!originalForm || !originalCategory) return false;
    // Compare all fields except picture (since it's always null unless changed)
    if (form.title !== originalForm.title) return true;
    if (form.description !== originalForm.description) return true;
    if (form.price !== originalForm.price) return true;
    if (form.delivery_time !== originalForm.delivery_time) return true;
    // Compare skills (array of objects with value)
    const origSkills = (originalForm.skills || []).map(s => s.value).sort();
    const currSkills = (form.skills || []).map(s => s.value).sort();
    if (origSkills.length !== currSkills.length || !origSkills.every((v, i) => v === currSkills[i])) return true;
    // Compare category
    if ((category?.value || null) !== (originalCategory?.value || null)) return true;
    // Compare picture (if a new file is selected)
    if (form.picture) return true;
    return false;
  }
  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-gradient-to-br from-[#1a1333] to-[#2d1a4d] text-white p-6 pt-28 flex">
        {/* Sidebar - identical to Create Gig */}
        ...
        <div className="flex-1 max-w-4xl w-full mx-auto">
          <form onSubmit={handleSubmit} className="w-full" id="edit-gig-form">
            {/* Basic Info Section */}
        ...
        {showSuccessModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-60 backdrop-blur-sm animate-fadeIn">
            <div className="bg-[#24194a] rounded-lg shadow-2xl p-8 relative animate-fadeIn flex flex-col items-center">
              <h2 className="text-2xl font-bold text-green-400 mb-4">Gig Updated Successfully!</h2>
              <p className="text-white mb-6">Your gig has been updated.</p>
              <button
